...
[`S1_SOLVER_BOUNDARY_PROFILE.md`](../s1_history/S1_SOLVER_BOUNDARY_PROFILE.md),
with raw evidence in `../evidence/s1/s1_solver_boundary_profile.json`.
...
`../tools/analyze_persisted_timings.py` reads the existing HDF5 tile evidence only; it
does not import or invoke dynamics:

```bash
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m \
  development.chaos_content.prototypes.state_space_maps.investigations.performance.tools.analyze_persisted_timings \
...
```bash
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m \
  development.chaos_content.prototypes.state_space_maps.investigations.performance.archive.probe_pool_lifecycle \
  --repeats 3
```
...
```bash
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m \
  development.chaos_content.prototypes.state_space_maps.investigations.performance.archive.probe_route_stratified_cells \
  development/chaos_content/prototypes/state_space_maps/outputs/finite_time_field/finite_time_field_1024.h5 \
  --selection-only
...
[`route_stratified_16_cells.json`](../evidence/s1/route_stratified_16_cells.json).
...
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m \
  development.chaos_content.prototypes.state_space_maps.investigations.performance.archive.probe_worker_lifetime
```
...
[`worker_lifetime_4096_cells.json`](../evidence/lifecycle/worker_lifetime_4096_cells.json).
...
[`worker_lifetime_ab_2048_cells.json`](../evidence/lifecycle/worker_lifetime_ab_2048_cells.json).
...
to
[`runner_recycling_candidate_design.json`](../evidence/lifecycle/runner_recycling_candidate_design.json),
...
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m \
  development.chaos_content.prototypes.state_space_maps.investigations.performance.archive.probe_runner_recycling_candidate \
  --design-only
...
[`runner_recycling_candidate_64.json`](../evidence/lifecycle/runner_recycling_candidate_64.json).
