"""Validated compiled DOP853 integration for the finite-time observable.
...
COMPILED_DOP853_EVALUATOR = "numba_rhs_compiled_dop853"
...
class _AcceptedStepSegment:
    """Internal accepted-step trajectory returned by one compiled segment."""
...
def _integrate_compiled_dop853_segment_unchecked(
    ...
    if solver.method.upper() != "DOP853":
        raise ValueError("The compiled DOP853 path supports only DOP853.")
    ...
    if final_state.shape != initial.shape or not np.all(np.isfinite(final_state)):
        raise RuntimeError("compiled DOP853 returned a non-finite or malformed state.")
    if not math.isclose(
        float(integrator.t),
        end,
        rel_tol=0.0,
        ...
        raise RuntimeError(
            "compiled DOP853 did not reach the requested segment endpoint: "
            f"{integrator.t} != {end}."
        )
    ...
    if (
        ...
        raise RuntimeError("compiled DOP853 accepted-step output is invalid.")
...
def _integrate_compiled_dop853_segment(
    rhs: Callable[[float, np.ndarray], np.ndarray],
    initial: np.ndarray,
    requested: np.ndarray,
    solver: SolverSpec,
...
def _solve_compiled_dop853_segment(
    rhs: Callable[[float, np.ndarray], np.ndarray],
    initial: np.ndarray,
    requested: np.ndarray,
    solver: SolverSpec,
...
def run_renormalized_tangent_compiled_dop853(
    spec: RenormalizedTangentSpec | None = None,
) -> RenormalizedTangentResult:
    """Run the accepted observable with Numba RHS/JVP and compiled DOP853."""

    spec = spec or RenormalizedTangentSpec()
    return _run_renormalized_tangent_with_rhs(
        spec,
        compiled_rhs(spec.parameters),
        segment_solver=_solve_compiled_dop853_segment,
    )


def evaluate_renormalized_tangent_compiled_dop853(
    ...
    return evaluate_renormalized_tangent_runner(
        spec,
        runner=run_renormalized_tangent_compiled_dop853,
