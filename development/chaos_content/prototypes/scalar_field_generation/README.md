# Scalar-field generation prototype

This package promotes the cross-observable computation boundaries accepted by
...
`work_units.py` plans `8 x 8` nominal half-open index rectangles with clipped
edges. `hdf5.py` owns the authoritative float64 values, uint8 status/route
arrays, provenance, tile completion markers, checksums, and fail-closed
reader. `runner.py` evaluates pending tiles through four spawn-isolated
...
write HDF5. `validation.py` reopens and validates an artifact without dynamics.
