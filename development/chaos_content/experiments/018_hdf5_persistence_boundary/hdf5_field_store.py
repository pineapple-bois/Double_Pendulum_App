"""Dynamics-independent HDF5 persistence boundary for scalar-field tiles."""
...
class SimulatedInterruption(PersistenceError):
    """Experiment-only interruption before the completion marker."""
...
class FieldDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        if np.dtype(self.scalar_dtype) != np.dtype("<f8"):
            raise ValueError("Experiment 018 supports the earned float64 scalar dtype.")
...
def write_completed_tile(
    ...
    *,
    ...
    if interrupt_after not in (None, "payload", "checksum"):
        raise ValueError("interrupt_after must be payload, checksum, or None.")
    with h5py.File(path, "r+") as output:
        ...
        if interrupt_after == "payload":
            raise SimulatedInterruption("Interrupted after tile payload flush.")
        ...
        output.flush()
        if interrupt_after == "checksum":
            raise SimulatedInterruption("Interrupted after tile checksum flush.")
...
def validate_dataset(path: Path) -> DatasetValidation:
    ...
    with h5py.File(path, "r") as source:
        ...
        for tile_index in resume.completed_tile_indices:
            ...
            if np.any(status == 0) or not set(int(value) for value in np.unique(status)).issubset({1, 2, 3}):
...
def _main() -> None:
    parser = argparse.ArgumentParser(
        description="Inspect an Experiment 018 scalar-field dataset without dynamics."
    )
    parser.add_argument("dataset", type=Path)
    arguments = parser.parse_args()
    print(json.dumps(inspect_dataset(arguments.dataset), indent=2, sort_keys=True))
