...
from development.chaos_content.prototypes.state_space_maps.src.lyapunov import compiled_dop853 as compiled_dop853_module
from development.chaos_content.prototypes.state_space_maps.src.lyapunov.compiled import compiled_rhs, run_renormalized_tangent_compiled
...
from development.chaos_content.prototypes.state_space_maps.src.lyapunov.compiled_dop853 import (
    ...
    _integrate_compiled_dop853_segment,
...
def promoted_comparisons():
    ...
    for angle_pair in VALIDATION_ANGLE_PAIRS_DEGREES:
        ...
        promoted = run_renormalized_tangent_compiled_dop853(spec)
        results.append(
...
def test_promoted_segment_observes_valid_accepted_steps() -> None:
    ...
    segment = _integrate_compiled_dop853_segment(
    ...
    assert np.max(np.diff(segment.time)) <= (
        promoted.diagnostics.max_step_seconds
        + compiled_dop853_module._MAX_STEP_FLOATING_POINT_ALLOWANCE
    )
...
def test_promoted_evaluator_preserves_valid_and_invalid_outcome_semantics() -> None:
    ...
    promoted_valid = evaluate_renormalized_tangent_compiled_dop853(valid_spec)
    assert oracle_valid.status is promoted_valid.status is EvaluationStatus.COMPLETED_VALID
    assert promoted_valid.evaluator == COMPILED_DOP853_EVALUATOR
    ...
    promoted_invalid = evaluate_renormalized_tangent_compiled_dop853(invalid_spec)
    assert (
...
def test_promoted_evaluator_bounds_runtime_errors_only(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        compiled_dop853_module,
        "_solve_compiled_dop853_segment",
        numerical_error,
    )
    outcome = evaluate_renormalized_tangent_compiled_dop853(
...
def test_low_level_numerical_failure_is_translated(monkeypatch) -> None:
    ...
    monkeypatch.setattr(compiled_dop853_module, "ode", lambda _rhs: FailedIntegrator())
    spec = RenormalizedTangentSpec(duration=0.25)
...
def test_low_level_specification_errors_propagate() -> None:
    ...
    with pytest.raises(ValueError, match="only DOP853"):
        _integrate_compiled_dop853_segment(
            ...
            np.ones(8),
            np.asarray((0.0, 0.25)),
            compiled_dop853_module.SolverSpec(method="RK45"),
            0.01,
        )
